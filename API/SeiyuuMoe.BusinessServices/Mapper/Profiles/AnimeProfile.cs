	...
	internal class AnimeProfile : Profile
	{
		...
		public AnimeProfile()
		{
			...
			CreateMap<Anime, AnimeCardDto>()
				...
				.ForMember(dest => dest.AiringDate, opt => opt.MapFrom(src => !string.IsNullOrWhiteSpace(src.AiringDate) ? DateTime.ParseExact(src.AiringDate, "dd-MM-yyyy", null) : (DateTime?)null))
			...
			CreateMap<Anime, AnimeTableEntryDto>()
				...
				.ForMember(dest => dest.MalId, opt => opt.MapFrom(src => src.MalId))
				.ForMember(dest => dest.ImageUrl, opt => opt.MapFrom(src => src.ImageUrl))
				.ForMember(dest => dest.AiringFrom, opt => opt.MapFrom(src => !string.IsNullOrWhiteSpace(src.AiringDate) ? DateTime.ParseExact(src.AiringDate, "dd-MM-yyyy", null) : (DateTime?)null))
